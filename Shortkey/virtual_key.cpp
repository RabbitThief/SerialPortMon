...
int getVkIndex (int vk)
{
	for (int i=0; i<256 && g_vk_list[i].vk != -1; ++i) {
		if (vk == g_vk_list[i].vk) return i;
	}
	return -1;
}

int GetVirtualKeyCode (int index)
{
	if (0 <= index && index <sizeof(g_vk_list)/sizeof(sVirtualKeyDesc)) {
		return g_vk_list[index].vk;
	}
	return -1;
}

const char *GetVirtualKeyDesc (int index)
{
	if (0 <= index && index <sizeof(g_vk_list)/sizeof(sVirtualKeyDesc)) {
		return g_vk_list[index].desc;
	}
	return "(Unknown)";
}

const char *GetVirtualKeyDescFromVK (int vk)
{
	int index = getVkIndex (vk);
	if (index >= 0) {
		return 	g_vk_list[index].desc;
	}
	return "(Unknown)";
...
const char *GetVirtualKeyNameFromVK (int vk)
{
	int index = getVkIndex (vk);
	if (index >= 0) {
		return 	g_vk_list[index].name;
	}
	return "(Unknown)";
}
